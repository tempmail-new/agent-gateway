...
For a compact steady-state snapshot between startup, traffic generation, and deeper inspection:

```bash
make observability-status
```

The status helper prints the compose project, compose file, local endpoint URLs, compose service state, gateway readiness, collector metric presence, Prometheus readiness, Grafana health, and Grafana dashboard availability without streaming logs or failing the command when a surface is unavailable.
