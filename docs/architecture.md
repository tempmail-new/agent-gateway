...
8. `withGatewayTrace` wraps provider execution with OpenTelemetry API attributes and events for provider duration, upstream status, timeout state, and normalized error codes.
...
- Provider-call logs intentionally exclude prompts, metadata, bearer tokens, and provider API keys. They include operational fields such as provider, model, request ID, duration, upstream status, timeout flag, and normalized error code.
